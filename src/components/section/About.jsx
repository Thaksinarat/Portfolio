                                ...
                                {myEducation.map((edu, key) => (
                                    <div key={key} className="py-1 px-3">
                                        <div className="flex items-between justify-between border-b border-white/10">
                                            <span className="text-bold md:text-lg text-amber-500">{edu.institution}</span>
                                            <span className="relative inline-flex h-3 w-3">
                                                <span
                                                    className={`absolute inline-flex h-full w-full rounded-full opacity-75 ${edu.graduated ? "" : "animate-ping bg-purple-500"
                                                        }`}
                                                ></span>
                                                <span
                                                    className={`relative inline-flex h-3 w-3 rounded-full ${edu.graduated ? "" : "bg-purple-500"
                                                        }`}
                                                ></span>
                                            </span>
                                        </div>
                                        ...
                                        <div className="flex items-between justify-between gap-2 pt-2">
                                            {/* Faculty & Major */}
                                            <div>
                                                <span>{edu.program}{edu.faculty} {edu.major} ({edu.firstYear}-{edu.graduateYear})</span>
                                            </div>
                                            {/* Grade */}
                                            <div>
                                ...
                                {myIntern.map((intern, key) => (
                                    <div key={key} className="py-1 px-3">
                                        <div className="border-b border-white/10">
                                            <span className="text-bold md:text-lg text-amber-500">{intern.company}</span>
                                        </div>
                                        <div className="flex items-between justify-between gap-2 pt-2">
                                            {/* Position & Department */}
                                            <div>
                                                <p>{intern.position}<span> ({intern.finalDate} - {intern.finalDate})</span></p>
                                            </div>
                                        </div>
                        ...
                        <div className="rounded-xl p-6">
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                {frontendSkills.map((skill, key) => (
                                    <span
                                        key={key}
                                        className="bg-purple-500/10 text-purple-500 py-1 px-3 rounded-full text-sm">
                                        {skill}
                                    </span>
                                ))}
                            </div>
